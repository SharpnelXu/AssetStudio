...
public class OFBStream : Stream
{
  ...
  public OFBStream(Stream parent, SymmetricAlgorithm algo, CryptoStreamMode mode)
  {
    ...
    this.parent = parent;
    cbcStream = new CryptoStream(new ZeroStream(), algo.CreateEncryptor(), CryptoStreamMode.Read);
    this.mode = mode;
  ...
  public override int Read(byte[] buffer, int offset, int count)
  {
    ...
    var toRead = Math.Min(count, readWriteBuffer.Length);
    var read = parent.Read(readWriteBuffer, 0, toRead);
    if (read == EOS)
      return EOS;
  ...
  {
    var read = cbcStream.Read(keyStreamBuffer, 0, keyStreamBuffer.Length);
